...
require_admin_role(['master'], $admin_base);
            ...
            try {
                if ($action === 'create') {
                    ...
                    if ($id <= 0) $error = 'Invalid method.';
                        ...
                        $old = $pdo->prepare("SELECT * FROM deposit_methods WHERE id=? AND owner_id=? AND target_role='agent'");
                        ...
                        if (!$before) $error = 'Method not found.';
                            ...
                            $stmt = $pdo->prepare("UPDATE deposit_methods SET label=?, channel=?, enabled=?, sort_order=?, details_json=? WHERE id=? AND owner_id=? AND target_role='agent'");
                    ...
                    if ($id <= 0) $error = 'Invalid method.';
                        ...
                        if (!$before) $error = 'Method not found.';
                            ...
                            $stmt = $pdo->prepare("DELETE FROM deposit_methods WHERE id=? AND owner_id=? AND target_role='agent'");
...
$upstream = [];
try {
    $stmt = $pdo->query("SELECT * FROM payment_modes WHERE enabled=1 AND FIND_IN_SET('master', REPLACE(COALESCE(allowed_roles,''),' ','')) > 0 ORDER BY name ASC");
    $upstream = $stmt->fetchAll() ?: [];
} catch (Exception $e) {
    $upstream = [];
}

$mine = $pdo->prepare("SELECT * FROM deposit_methods WHERE owner_id=? AND target_role='agent' ORDER BY sort_order ASC, id DESC");
$mine->execute([$my_id]);
$mine = $mine->fetchAll();
?>
  ...
  <div class="col-12 col-lg-6">
    <div class="card">
      <div class="card-header">
        <h5 class="mb-1">Deposit Methods</h5>
        <div class="text-body-secondary">Payment modes set by admin</div>
      </div>
      <div class="card-body">
        <?php if (!empty($upstream)): ?>
          <div class="row g-3">
            <?php foreach ($upstream as $m): ?>
              <?php $d = json_assoc($m['config_json'] ?? ''); ?>
              <?php
              ...
              <div class="col-12">
                <div class="border rounded p-3">
                  <?php
                    $qr = trim((string)($d['qr_path'] ?? ($d['qr_url'] ?? '')));
                    $qr_href = $qr;
                    if ($qr_href !== '' && !preg_match('#^https?://#i', $qr_href) && $qr_href[0] !== '/') $qr_href = $admin_base . $qr_href;
                  ?>
                  <div class="d-flex align-items-start justify-content-between gap-3">
                    <div class="d-flex align-items-start gap-2">
                      <i class="<?php echo htmlspecialchars($icon); ?>" style="font-size: 18px; line-height: 1;"></i>
                      <div>
                        <div class="fw-semibold"><?php echo htmlspecialchars((string)($m['name'] ?? '')); ?></div>
                        <div class="text-body-secondary small"><?php echo htmlspecialchars(strtoupper($channel)); ?></div>
                      </div>
                    </div>
                    <?php if ($channel === 'upi' && $qr_href !== ''): ?>
                      <a href="<?php echo htmlspecialchars($qr_href); ?>" target="_blank" rel="noopener">
                        <img src="<?php echo htmlspecialchars($qr_href); ?>" alt="QR" style="width: 92px; height: 92px; object-fit: cover; border-radius: 10px; border: 1px solid rgba(0,0,0,0.08); background: #fff;">
                      </a>
                    <?php endif; ?>
                  </div>
                  ...
                  <div class="mt-3 row g-2">
                    <?php if ($channel === 'upi'): ?>
                      <?php if (!empty($d['upi_id'])): ?>
                        <div class="col-12 col-md-6">
                          <div class="small text-body-secondary">UPI ID</div>
                          <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['upi_id']); ?></div>
                        </div>
                      <?php endif; ?>
                      <?php if (!empty($d['holder_name'])): ?>
                        <div class="col-12 col-md-6">
                          <div class="small text-body-secondary">Holder</div>
                          <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['holder_name']); ?></div>
                        </div>
                      <?php endif; ?>
                    <?php elseif ($channel === 'bank'): ?>
                      <?php if (!empty($d['bank_name'])): ?>
                        <div class="col-12 col-md-6">
                          <div class="small text-body-secondary">Bank</div>
                          <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['bank_name']); ?></div>
                        </div>
                      <?php endif; ?>
                      <?php if (!empty($d['account_number'])): ?>
                        <div class="col-12 col-md-6">
                          <div class="small text-body-secondary">A/C No.</div>
                          <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['account_number']); ?></div>
                        </div>
                      <?php endif; ?>
                      <?php if (!empty($d['ifsc'])): ?>
                        <div class="col-12 col-md-6">
                          <div class="small text-body-secondary">IFSC</div>
                          <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['ifsc']); ?></div>
                        </div>
                      <?php endif; ?>
                      <?php if (!empty($d['account_name'])): ?>
                        <div class="col-12 col-md-6">
                          <div class="small text-body-secondary">Account Name</div>
                          <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['account_name']); ?></div>
                        </div>
                      <?php endif; ?>
                    <?php elseif ($channel === 'wallet'): ?>
                      <?php if (!empty($d['wallet_name'])): ?>
                        <div class="col-12 col-md-6">
                          <div class="small text-body-secondary">Wallet</div>
                          <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['wallet_name']); ?></div>
                        </div>
                      <?php endif; ?>
                      <?php if (!empty($d['wallet_address'])): ?>
                        <div class="col-12 col-md-6">
                          <div class="small text-body-secondary">Address</div>
                          <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['wallet_address']); ?></div>
                        </div>
                      <?php endif; ?>
                    <?php else: ?>
                      <?php if (!empty($d['instructions'])): ?>
                        <div class="col-12">
                          <div class="small text-body-secondary">Instructions</div>
                          <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['instructions']); ?></div>
                        </div>
                      <?php endif; ?>
                      <?php if (!empty($d['url'])): ?>
                        <div class="col-12">
                          <div class="small text-body-secondary">URL</div>
                          <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['url']); ?></div>
                        </div>
                      <?php endif; ?>
                    <?php endif; ?>
                    <?php if (!empty($d['notes'])): ?>
                      <div class="col-12">
                        <div class="small text-body-secondary">Notes</div>
                        <div class="fw-semibold"><?php echo htmlspecialchars((string)$d['notes']); ?></div>
                      </div>
                    <?php endif; ?>
                  </div>
                </div>
        ...
        <?php else: ?>
          <div class="text-center text-body-secondary py-4">No payment modes configured.</div>
        <?php endif; ?>
      </div>
    </div>
  </div>
  ...
  <div class="col-12 col-lg-6">
    <div class="card mb-4">
      <div class="card-header">
        ...
        <div class="text-body-secondary">Visible to agents</div>
      </div>
      <div class="card-body">
    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="text-body-secondary">Your methods for agents</div>
      </div>
      <div class="card-body">
        <?php if (!empty($mine)): ?>
