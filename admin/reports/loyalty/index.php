...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_loyalty'])) {
    if (!csrf_check($_POST['csrf_token'] ?? '')) {
        ...
        if ($uid <= 0) {
            ...
            $stmt = $pdo->prepare("INSERT INTO loyalty_ledger (user_id, points, note, created_by) VALUES (?, ?, ?, ?)");
            if ($stmt->execute([$uid, $points, $note !== '' ? $note : null, (int)($_SESSION['user_id'] ?? 0)])) {
                audit_log($pdo, 'loyalty_add', 'loyalty_ledger', (string)$pdo->lastInsertId(), null, ['user_id' => $uid, 'points' => $points]);
                $message = 'Loyalty entry added.';
            } else {
                $error = 'Insert failed.';
            }
        }
...
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM loyalty_ledger l" . $where);
$countStmt->execute($params);
$total = (int)$countStmt->fetchColumn();

$sumStmt = $pdo->prepare("SELECT COALESCE(SUM(l.points),0) FROM loyalty_ledger l" . $where);
...
$sql = "SELECT l.*, u.username
        ...
        $where
        ORDER BY l." . $filters['sort'] . " " . strtoupper($filters['dir']) . "
...
if (in_array($filters['export'], ['csv', 'excel', 'pdf'], true)) {
    ...
    if ($filters['export'] === 'pdf') {
        ...
        echo '<html><head><title>Loyalty Report</title></head><body>';
        echo '<h3>Loyalty Report</h3>';
    ...
    $filename = $filters['export'] === 'excel' ? 'loyalty_report.xls' : 'loyalty_report.csv';
...
<div class="card mb-4">
  <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-3">
    <div>
      <h5 class="mb-1">Loyalty Report</h5>
  ...
  <div class="card-body">
    ...
    <form method="POST" class="row g-3 align-items-end mb-4">
      ...
      <input type="hidden" name="add_loyalty" value="1">
      <div class="col-12 col-md-2">
